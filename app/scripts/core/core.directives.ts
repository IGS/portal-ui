        ...
        this.$onInit = function () {
          let openLogin = () => {
            if (navigator.cookieEnabled) {
              ...
              const redirectUrl = config['site-wide']['auth'] +
              ...
              const closeLogin = (url) => {
                if (url === redirectUrl) {
                  ...
                  return (<any>_).includes(url, returningPath);
                }
              }
              ...
              const interval = setInterval(() => {
                try {
                  ...
                  else if (closeLogin((<any>_).get(win, 'document.URL', ''))) {
                    win.close();
                    setTimeout(() => {
                      clearInterval(interval);
                      UserService.login();
  ...
  function logoutButton(config): ng.IDirective {
    return {
      ...
      controller: function ($scope, $element, $window) {
        this.$onInit = function () {
          let logout = () => {
            var redirect = config['site-wide']['auth'];
            var authQuery = "";
